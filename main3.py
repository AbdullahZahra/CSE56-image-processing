...
def apply_filter(filter_type):
    ...
    if filter_type == 'Close':
        ...
        filtered_img = cv2.morphologyEx(filtered_img, cv2.MORPH_CLOSE, kernel)
    elif filter_type == 'Hough Transform for Circle':
        filtered_img = img  # Placeholder, this filter doesn't change the image
    elif filter_type == 'Segmentation using Region Split and Merge':
        filtered_img = img  # Placeholder, this filter doesn't change the image
    elif filter_type == 'Segmentation using Thresholding':
        filtered_img = img  # Placeholder, this filter doesn't change the image
...
filter_buttons = [widgets.Button(description=filter_type) for filter_type in ['Close', 'Hough Transform for Circle', 'Segmentation using Region Split and Merge', 'Segmentation using Thresholding']]
