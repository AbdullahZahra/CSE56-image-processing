...
def apply_filter(filter_type):
    ...
    if filter_type == 'HPF':
        filtered_img = cv2.filter2D(filtered_img, -1, kernel_HPF)
    elif filter_type == 'LPF':
        filtered_img = cv2.filter2D(filtered_img, -1, kernel_LPF)
    elif filter_type == 'Mean Filter':
        filtered_img = cv2.blur(filtered_img, (5, 5))
    elif filter_type == 'Median Filter':
        filtered_img = cv2.medianBlur(filtered_img, 5)
    elif filter_type == 'Robert Edge Detector':
        filtered_img = cv2.filter2D(filtered_img, -1, kernel_robert)
...
filter_buttons = [widgets.Button(description=filter_type) for filter_type in ['HPF', 'LPF', 'Mean Filter', 'Median Filter', 'Robert Edge Detector']]
